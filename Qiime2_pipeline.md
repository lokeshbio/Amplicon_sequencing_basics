...
The EMP (Earth Microbiome Project) format always contains the amplicon reads in two files: "reads.fq.gz" file and the "barcodes.fq.gz" file. We can get these two files separately from a single fastq file from any Illumina or Ion-Torrent machine by using the commnad from Qiime1 "extract\_barcodes.py".
